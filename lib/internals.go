...
func (m Mail2Most) processReader(mr *gomail.Reader, profile int) (string, []Attachment, bool, error) {
	...
	for {
		...
		if (len(html) > 0) && (len(text) > 0) {

			if m.Config.Profiles[profile].Mattermost.PreferPlaintext {
				body = text
			...
			body = text
			m.Debug("TEXT only mail", map[string]interface{}{})

		} else if len(html) > 0 {
			body = html
			wasHTML = true
			m.Debug("HTML only mail", map[string]interface{}{})

		} else {
			body = "no mail body content found"
		}
	}
